...
function TabContact({ delay }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) { setStatus('success'); setForm({ name:'', email:'', subject:'', message:'' }); track('contact_form_submit', { subject: form.subject }); }
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          setLoc(detected); track('location_detected', { method: 'gps', city: detected.name });
        ...
        <div className="tab-bar">
          {TABS.map(t => (
            <button key={t.id} className={`tab ${tab===t.id?'active':''}`}
              onClick={() => { setTab(t.id); track('tab_switch', { tab: t.id }); }}>
              {t.icon} {t.label}
            </button>
          ))}
        </div>
